...
function gestionlibros()
 ...
 FROM libros lib INNER JOIN  editorial edi on lib.editorial = edi.id_editorial ";
    ...
    $query->bindParam(1, $idlib);
    $query->execute(); //fetch per rol
     $aaa = "<table id="."tabla"." border=1 style='max-heigth:150px'> ";
    $aaa.= ('<tr>' . '<td>' . 'id' . '</td>' . '<td>' . 'Nombre Libro' . 
    '</td>' . '<td>' . 'ISBN' . '</td>' . '<td>' . 'Idioma' . '</td>' . 
 ...
 if ($query->rowCount() >= 1)
    ...
    while ($fila = $query->fetch(PDO::FETCH_ASSOC))
      ...
      $aaa.= $form;
      $aaa.= ('<tr><td>' . utf8_encode($fila['id_libro']) . 
        '</td><td>' . utf8_encode($fila['nombre']) . 
        '</td><td>' . utf8_encode($fila['ISBN']) . 
        '</td><td>' . utf8_encode($fila['idioma']) . 
        '</td><td>' . utf8_encode($fila['imagen']) . 
        '</td><td>' . utf8_encode($fila['description']) .
    ...
    $aaa.=('<form class="registre" name="insertar" method="post" accept-charset="UTF-8" action="' . APP_W . 'lib">
    <input type="submit" name="id" value="Insertar un nuevo libro">
    </form>');
